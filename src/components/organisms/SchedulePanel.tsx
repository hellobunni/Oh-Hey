...
function SchedulePanel({
  ...
  return (
    ...
    >
      <div className="mb-2 font-px text-sm text-peri">{title}</div>
...
function ScheduleRow({ item }: { item: ScheduleItem }) {
  ...
  const rowClass = cn(
    ...
    today &&
      'animate-[today-pulse_1.2s_ease-out_1] border-l-[3px] border-mint bg-mint-soft font-bold text-white',
  ...
  const content = (
    <>
      <span className="flex items-baseline gap-1.5">
        <span>{day}</span>
        {today && (
          <span className="font-sans text-[10px] font-bold uppercase tracking-wide text-mint">
            TODAY
          </span>
        )}
      </span>
      ...
      >
        ...
        {interactive && (
          <button
            ...
            className={cn(
              'shrink-0 cursor-pointer border-0 bg-transparent p-0.5 text-mint',
