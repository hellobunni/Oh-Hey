  ...
  return (
    ...
    >
      ...
      <div className="relative h-40 overflow-hidden bg-deep">
        ...
        {!thumbnail && !loading && (
          <div className="absolute inset-0 flex items-center justify-center font-px text-[10px] uppercase tracking-widest text-ink-mute">
            Thumbnail
          </div>
        )}

        {/* Duration — stays fixed (no zoom with thumb) */}
        <span className="absolute bottom-2 right-2 z-10 bg-black/60 px-1.5 py-0.5 font-px text-[10px] text-white">
          {duration}
        </span>
      ...
      <div className="flex gap-2.5 p-3.5">
        {channelHref ? (
          ...
          >
            <Avatar src={avatarSrc} size={32} ring="mint" />
          </button>
        ) : (
          <Avatar src={avatarSrc} size={32} ring="mint" />
        )}
