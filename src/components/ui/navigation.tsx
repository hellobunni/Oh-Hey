...
const navContainerVariants = cva("flex items-center h-14 border-b-2 px-6", {
  variants: {
    brand: {
...
const navBrandVariants = cva(
  "flex items-center gap-2.5 no-underline whitespace-nowrap shrink-0",
  {
    variants: {
      brand: {
        "oh-hey-lynae": "font-pixel text-sm text-primary",
      },
    },
  },
);
...
const navCtaVariants = cva(
  "shrink-0 no-underline cursor-pointer transition-all duration-100",
  {
    variants: {
      brand: {
        "oh-hey-lynae":
          "font-pixel text-xs bg-accent text-ink py-2.5 px-3.5 shadow-[3px_3px_0_rgba(0,0,0,0.35)] hover:bg-primary hover:-translate-x-px hover:-translate-y-px hover:shadow-[4px_4px_0_rgba(0,0,0,0.35)]",
      },
    },
  },
);
  ...
  return (
    ...
    >
      ...
      {!mobile && (
        <div
          className={cn(
            "flex items-center gap-0.5 flex-1 px-6",
            linksAlignClass[linksAlign],
          )}
        >
          {links.map((link) => (
            <a
...
function MobileMenu({
  ...
  return (
    <div
      className={cn(
        "bg-paper border-x-2 border-b-2 border-card flex flex-col",
        className,
      )}
    >
      {items.map((item) => {
