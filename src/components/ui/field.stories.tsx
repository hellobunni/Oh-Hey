...
function ThemePanel({
  ...
  return (
    ...
    >
      <div
        className={`font-pixel text-xs ${theme === "light" ? "text-ink" : "text-primary"}`}
      >
...
function TextareaWithCounter({ maxLength = 240 }: { maxLength?: number }) {
  const defaultVal =
    "We're rebuilding our admin app and need a design system that'll outlast the next two redesigns.";
  ...
  if (isSubmitSuccessful)
    return (
      <div className="flex flex-col gap-3 font-sans text-sm font-semibold text-ink-soft">
        ...
        <button
          onClick={() => reset()}
          className="cursor-pointer border-none bg-transparent p-0 text-left font-sans text-sm font-semibold text-accent underline"
        >
          Send another →
        </button>
      </div>
  ...
  render: () => (
    <div className="flex flex-col gap-7 bg-deep p-12 text-paper-ink">
      <div className="font-pixel text-[22px] text-primary">
        FORM FIELDS — ALL TYPES & VARIANTS
      </div>
      <p className="max-w-[760px] font-sans text-sm font-semibold text-ink-soft">
        Text input, select, textarea, checkbox, radio, switch, choice card,
        search, dropzone. Quicksand throughout — form UI is never pixel type.
      </p>
      ...
      {/* Select & textarea */}
      <div className="mt-3 border-b-2 border-card pb-2 font-pixel text-[15px] text-accent">
        SELECT & TEXTAREA
      </div>
      <div className="flex flex-wrap gap-8">
...
export const FieldTypes: Story = {
  ...
  render: () => (
    <div>
      ...
      <h2 className="mb-1 font-pixel text-2xl text-primary">FORM FIELDS</h2>
      <p className="mb-8 max-w-2xl font-sans text-sm leading-relaxed text-ink-soft">
        Quicksand throughout. Focus holds mint; error is red in light / pink in
        dark. Borders and surfaces flip via paper/ink tokens.
      </p>
...
export const ComposedForm: Story = {
  ...
  render: () => (
    <div className="flex max-w-md flex-col gap-6">
      <div className="rounded-xl border border-line bg-paper-2 p-5">
        <Field
          label="Get the digest"
          helper="every other Sunday · no tracking"
          suffix="SUBSCRIBE →"
          suffixAction
        >
          <input placeholder="your@email.com" />
