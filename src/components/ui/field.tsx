...
const controlVariants = cva(
  ...
  ].join(" "),
  {
    variants: {
      state: {
        default:
          "border-line-strong dark:border-ink-mute focus-within:border-primary",
        focus: "border-primary",
        error:
          "border-red-600 dark:border-alert focus-within:border-red-600 dark:focus-within:border-alert",
        success: "border-[#059669] focus-within:border-[#059669]",
        disabled:
          "border-line-strong/60 dark:border-transparent bg-paper-2 dark:bg-card opacity-100",
      },
      layout: {
        stacked: "",
        inline: "flex-1",
  ...
  "flex-1 min-w-0 font-sans font-semibold text-ink bg-transparent border-0 outline-none placeholder:text-ink-mute",
  {
    variants: {
      size: {
        ...
        md: "px-3.5 py-2.5 text-sm", // 10×14
      ...
      state: {
        ...
        success: "",
        disabled:
          "text-ink-mute dark:text-ink-soft cursor-not-allowed pointer-events-none",
      },
    },
...
function Field({
  ...
  const resolvedState: FieldState | "default" = error
    ? "error"
    : (state ?? "default");
    ...
    <div className={cn(wrapperVariants({ layout }), className)}>
      <label
        className={labelVariants({
          state: resolvedState as FieldState,
          layout,
        })}
        htmlFor={inputId}
      >
        {label}
        {required && (
          <span className="text-red-600 dark:text-alert" aria-hidden="true">
            *
          </span>
        )}
      </label>
      ...
      >
        {prefix && (
          <span
            className={cn(
              adornmentVariants({ size, action: !!prefixAction }),
              "rounded-l-[6px] border-r-2 border-line-strong dark:border-ink-mute",
            )}
        ...
        {suffix && (
          <span
            className={cn(
              adornmentVariants({ size, action: !!suffixAction }),
              "rounded-r-[6px] border-l-2 border-line-strong dark:border-ink-mute",
            )}
      ...
      {error && (
        <span
          id={`${inputId}-err`}
          className={subLabelVariants({ type: "error" })}
          role="alert"
        >
          {error}
        </span>
      ...
      {success && (
        <span
          id={`${inputId}-ok`}
          className={subLabelVariants({ type: "success" })}
        >
          {success}
        </span>
      )}
      {helper && (
        <span
          id={`${inputId}-hint`}
          className={subLabelVariants({ type: "helper" })}
        >
          {helper}
        </span>
      )}
      {maxLength != null && currentLength != null && (
        <span className={subLabelVariants({ type: "counter" })}>
