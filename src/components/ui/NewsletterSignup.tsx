...
function NewsletterSignup({ onSubscribe, headline, body, className }: NewsletterSignupProps) {
  ...
  return (
    <div className={cn('text-center', className)}>
      ...
      {status === 'done' ? (
        <p className="mt-7 inline-flex items-center justify-center gap-2 font-px text-sm text-mint">
        ...
        <form onSubmit={handleSubmit} className="mt-7 flex shadow-[5px_5px_0_rgba(0,0,0,0.4)] max-w-[480px] mx-auto">
          ...
          <button
            ...
            className="font-px text-xs bg-mint text-ink border-0 px-5 cursor-pointer transition-colors hover:bg-pink disabled:opacity-50 whitespace-nowrap"
          >
            {status === 'loading' ? '...' : '+1UP'}
          </button>
        </form>
